...
| --- | --- | --- | --- |
| `github-daily-tracker-dispatch` | `daily-tracker.yml` | 周二至周六 `18:30` | 生成 Nasdaq-100 / QQQ 日报并发送邮件 |
| `github-tiingo-cache-backfill-dispatch` | `tiingo_cache_backfill.yml` | 周一至周五 `11:00`、`13:10`、`15:20` | 维护 Tiingo 历史价格缓存 |

## 3. 函数代码

两个函数使用相同 Python 代码，通过环境变量决定触发哪个 workflow。
...
- `daily-tracker.yml` 和 `tiingo_cache_backfill.yml` 保留 `workflow_dispatch`，不再保留 GitHub `schedule`。
